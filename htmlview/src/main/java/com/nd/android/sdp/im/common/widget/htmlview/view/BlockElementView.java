...
class BlockElementView extends AbstractElementView {
    ...
    public BlockElementView(Context context) {
        super(context);
    }
    ...
    public BlockElementView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
            ...
            for (int i = 0; i < children.size(); i++) {
                ...
                if (child instanceof TextFragmentView) {
                    ...
                    if(text.trim().contains(" ")) {
                        ...
                        wordWidth = HtmlUtils.measureText(font, text, wordStart, text.length());
                        minW = Math.max(minW, wordWidth);
                    }
                    minW = Math.max(minW, wordWidth);
                    currentLineWidth += wordWidth;
